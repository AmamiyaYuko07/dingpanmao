...
- **点击看大图**：点格子弹出当日分时和近 60 日 K 线，支持十字光标读数（可在设置里关闭）
- **AI 分析**：接入任意兼容 OpenAI Responses API 的服务（默认 DeepSeek），直接给出**买入 / 增持 / 持有观望 / 减仓 / 清仓**的操作建议，附建议仓位、关键价位和止损参考；支持多轮追问，可查看模型思考过程。用你自己的密钥，程序不内置也不上传
...
AI 功能需要**你自己的 API Key**，程序不提供也不内置任何密钥：

1. 打开设置 → **AI 分析**，填入 API Key，打开开关
2. 默认接口 `https://api.deepseek.com`、模型 `deepseek-v4-flash`；也可以改成任意兼容 OpenAI Responses API 的服务（例如 OpenAI 官方填 `https://api.openai.com/v1`）
3. 密钥以明文保存在 `%LOCALAPPDATA%\DingPanMao\settings.json`，只存在你本机，不会随程序分发

**联网搜索**：开关打开后，OpenAI 端点走它原生的 `web_search` 工具；DeepSeek 等端点会自动改用内置的财经资讯检索（免费、无需额外密钥）。DeepSeek 的 `tools` 参数只支持 `function`，其他类型会被忽略，所以走的是后者。
...
仓库里不包含编译好的可执行文件（`dist/` 已被忽略），需要自己构建：
